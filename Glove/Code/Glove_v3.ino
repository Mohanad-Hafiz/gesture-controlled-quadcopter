//****  IMPORTANT  ******
// Change the pin numbers according to the new schematic (radio pins + analog pins)
...
int throttle_up_threshold = 100;
int throttle_down_threshold = 50;
int yaw_up_threshold = 100;
int yaw_down_threshold = 50;
...
void loop()
{
  ...
  // if index finger is raised, throttle goes up
  if (analogRead(A0) > throttle_up_threshold) {
    ...
    // if index finger is pinched, throttle goes down
    data.throttle -= tg;
    data.throttle = constrain(data.throttle, 0, 255);
  }

  // if pinky finger is pinched, yaw clockwise
  if (analogRead(A1) < yaw_up_threshold) {
    data.yaw += yg;
    data.yaw = constrain(data.yaw, 0, 255)
  } else if (analogRead(A2) < yaw_down_threshold) {
    // if ring finger is pinched, yaw counter clockwise
    data.yaw -= yg;
    data.yaw = constrain(data.yaw, 0, 255)
  }
